...
with st.sidebar.expander("Option 2: User input form"):
    with st.form("inputs_form"):
        holiday = st.selectbox('Choose whether today is a designated holiday or not', options=['None', 'Columbus Day', 'Veterans Day', 'Thanksgiving Day', 'Christmas Day', 'New Years Day', 'Washingtons Birthday', 'Memorial Day', 'Independence Day', 'State Fair', 'Labor Day', 'Martin Luther King Jr Day'])
        temp = st.number_input('Average Temperature in Kelvin', min_value=0.0, max_value=350.0, value=0.0, step=1.0)
        rain_1h = st.number_input('Amount of mm of rain that occurred in an hour', min_value=0.0, max_value=1000.0, value=0.0, step=10.0)
        snow_1h = st.number_input('Amount of mm of snow that occurred in an hour', min_value=0.0, max_value=10.0, value=0.0, step=0.01)   
...
missing_columns = set(encode_dummy_df.columns) - set(user_encoded_df.columns)
...
# User input from uploaded CSV (if any)
if user_csv is not None:
    user_df = pd.read_csv(user_csv)
    default_df.drop(columns=['traffic_volume'], inplace=True)

    # Combine default_df and user_df
    encode_df = pd.concat([default_df, user_df])
    
    # Align columns with model feature names
    encode_dummy_df = pd.get_dummies(encode_df)
    
    # Align columns to match the model's feature names
    for col in bst_model.feature_names_in_:
        if col not in encode_dummy_df.columns:
            encode_dummy_df[col] = 0  # Add missing columns with zero values
            
    encode_dummy_df = encode_dummy_df[bst_model.feature_names_in_]  # Reorder columns to match model input

    user_encoded_df = encode_dummy_df.tail(len(user_df))  # Get user data
    ...
    user_df['Lower CI Limit'] = user_df['Lower CI Limit'].apply(lambda x: max(0, x))  # Ensure non-negative
    ...
    st.metric(label="Predicted Traffic Volume", value=f"{pred_value}")
    st.write("With a", (1 - alpha) * 100, "% confidence interval:")
